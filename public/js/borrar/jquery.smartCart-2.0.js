...
(function ($) {
    $.fn.smartCart = function (options) {
        ...
        return this.each(function () {
            ...
            // Labels & Messages              
            var labelCartMenuName = 'Registros seleccionados (_COUNT_)';  // _COUNT_ will be replaced with cart count
            var labelCartMenuNameTooltip = "Lista  Total Registros: _PRODUCTCOUNT_ "; // Lista | --- | Total cantidad: _ITEMCOUNT_
            var labelProductMenuName = 'Registros';
            var labelSearchButton = "Buscar";
            var labelSearchText = "Buscar";
            var labelCategoryText = "Categoría";
            var labelClearButton = "Limpiar";
            var labelAddToCartButton = " "; // Agregar a lista
            var labelQuantityText = "Cantidad";
            var labelProducts = 'Registros';
            var labelPrice = ' '; //Precio
            var labelSubtotal = ' '; //Subtotal
            var labelTotal = ' ';//Total
            var labelRemove = ' ';
            var labelCheckout = 'Listar';

            var messageConfirmRemove = 'Desea remover "_PRODUCTNAME_" de la lista?'; //  _PRODUCTNAME_ will be replaced with actula product name
            var messageCartEmpty = "El listado está vacío";
            var messageProductEmpty = "No hay registros para mostrar";
            var messageProductAddError = "No se pudo agregar el registro";
            var messageYaenLista = "Ya está en la lista";
            var messageItemAdded = 'Se agregó el registro a la lista';
            var messageItemRemoved = 'El registro fué removido de la lista';
            var messageQuantityUpdated = 'Se actualizó la cantidad del registro';
            var messageQuantityErrorAdd = 'Cantidad inválida. El registro no se agregó';
            var messageQuantityErrorUpdate = 'Cantidad inválida. La cantidad no se actualizó';
            ...
            if (options.enableSearch) {
                var btSearch = $('<a>' + labelSearchButton + '</a>').attr("href", "#").addClass("scSearch").attr("title", "Buscar Registro");
                $(btSearch).click(function () {
                    var searcStr = $(txtSearch).val();
                    populateProducts(searcStr);
                    return false;
                });
                var btClear = $('<a>' + labelClearButton + '</a>').attr("href", "#").addClass("scSearch").attr("title", "Borrar búsqueda");
            ...
            // var elmCartHeaderTitle2 = ""; // $('<label>'+labelPrice+'</label>').addClass("scCartTitle scCartTitle2"); YOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO
            // var elmCartHeaderTitle3 = $('<label>'+labelQuantityText+'</label>').addClass("scCartTitle scCartTitle3");
            // var elmCartHeaderTitle4 = ""; // $('<label>'+labelSubtotal+'</label>').addClass("scCartTitle scCartTitle4"); /* YOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO*/
            // var elmCartHeaderTitle5 = $('<label></label>').addClass("scCartTitle scCartTitle5");
            // .append(elmCartHeaderTitle2).append(elmCartHeaderTitle3).append(elmCartHeaderTitle4).append(elmCartHeaderTitle5)
            elmCartHeader.append(elmCartHeaderTitle1);
            ...
            var elmBottomSubtotalText = $('<label>' + labelTotal + '</label>').addClass("scLabelSubtotalText"); // : /*//*/*/*/*/*/*/*/*/YOOOOOOOOOOOOOOOOOOOOOO
            ...
            // $("#gua_accinc").click(function () {     
                if ($.isFunction(options.onCheckout)) {
                    // calling onCheckout event;
                    options.onCheckout.call(this, elmProductSelected);
                } else {
                    $(this).parents("form").submit();
                    // $("#gscf_accincedit").submit();
                }
                return false;
            });
            elmBottomBar.append(btCheckout);// .append(elmBottomSubtotalValue).append(elmBottomSubtotalText) /*/*/*/*/*/*/*/*/*/YOOOOOOOOOOOOOOOOOOOOOOOOOO
            // obj.append(elmBottomBar); YEOCH Para quitar botón listar 11-02-2016
            ...
            function addToCart(i, qty) {
                ...
                if (addProduct.length > 0) {
                    ...
                    if (productItem.length > 0) {
                        showHighlightMessage(messageYaenLista);

                        /* Comentado por mi para controlar solo cantidad de 1 11-02-2016
                         ...
                         var lblTotal =  ""; //$('#lblTotal'+i).html(prdTotal);//YOOOOOOOOOOOOOOOOOOOO
            ...
            function addCartItemDisplay(objProd, Quantity) {
                ...
                var elmCPTitle2 = ""; // $('<label>'+pPrice+'</label>').addClass("scCartItemTitle scCartItemTitle2"); YOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO
                var inputQty = $('<input type="text" value="' + Quantity + '" style="display: none;" />').attr("id", "lblQuantity" + pIndex).attr("rel", pIndex).addClass("scTxtQuantity2");
                ...
                var btRemove = $('<a >' + labelRemove + '</a>').attr("rel", pIndex).attr("href", "#").addClass("icon-minus-sign-alt").attr("title", "Remover de la lista");
                ...
                elmCPTitle1.append(elmCPContent);
                elmCP.append(elmCPTitle1).append(elmCPTitle3).append(elmCPTitle5); // .append(elmCPTitle2).append(elmCPTitle4)
            ...
            function fillCategory() {
                ...
                var catItem = $(''); // _________________________________________________________________________________________________________________________________________________________
            ...
            function populateProducts(searchString) {
                ...
                $(products).each(function (i, n) {
                    ...
                    if (isValid && isCategoryValid) {
                        ...
                        var btAddToCart = $('<a title="Agregar registro a la lista">' + labelAddToCartButton + '</a>').attr("href", "#").attr("rel", i).addClass("icon-plus-sign-alt").attr("title", "Agregar a la lista"); // .attr("title", labelAddToCartButton)
                        ...
                        var inputQty = $('<input type="text" value="1" style="display: none;" />').addClass("scTxtQuantity");
                        var labelQty = $('<label>' + labelQuantityText + ':</label>').addClass("scLabelQuantity");
                        elmProdDiv3.append(inputQty).append(btAddToCart);   //                 append(labelQty).
    ...
    $.fn.smartCart.defaults = {
        ...
        enableImage: false, //true,
        enableImageTooltip: true,
        enableSearch: true,
        enableCategoryFilter: false, //true
        productItemTemplate: '<strong><%=pname%></strong><br />', //Categoría: <%=pcategory%><br /><small><%=pdesc%></small><br /> <strong>Price: <%=pprice%></strong>
